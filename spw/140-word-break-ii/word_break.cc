...
class Solution {
public:
    void dfs(vector<string> &ans, vector<vector<int>> &adj, string &s, string now, int len) {
        if (len == 0) {
            ans.push_back(now);
            return;
        }
        ...
        if (now != "") now = " " + now;
        for (int i : adj[len]) {
            // keep in mind that the start point is exactly i
            dfs(ans, adj, s, s.substr(i, len-i)+now, i);
        }
    }
    vector<string> wordBreak(string s, unordered_set<string>& wordDict) {
        int n = s.size();
