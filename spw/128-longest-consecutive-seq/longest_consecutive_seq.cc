...
public:
    int longestConsecutive(vector<int>& nums) {
        ...
        for (int i = 0; i < n; ++i) {
            if (len.find(nums[i]) == len.end()) {
                len[nums[i]] = 1;
                int left = len.find(nums[i]-1) == len.end() ? 0 : len[nums[i]-1];
                int right = len.find(nums[i]+1) == len.end() ? 0 : len[nums[i]+1];
                len[nums[i]-left] = len[nums[i]+right] = 1 + left + right;

                ans = max(ans, len[nums[i]-left]);
            }
        }
