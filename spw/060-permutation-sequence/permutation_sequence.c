...
public:
    string getPermutation(int n, int k) {
        static vector<int> fac(2, 1);

        // n elements : 1 1 2 6 24 ... (n-1)!
        while (fac.size() < n) {
        ...
        for (int i = n; i >= 1; --i) {
            int d = findNext(used, 1);
            //if fac[i-1] == k then just select first unused one
            while (fac[i-1] < k) {
                k -= fac[i-1];
                d = findNext(used, d+1);
            }
    ...
    int findNext(vector<bool> &used, int i) {
        while (i < used.size()) {
            if (!used[i])
                return i;
            ++i;
